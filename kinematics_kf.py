...
class kalman_filter():
    ...
    def __init__(self, node_name, frequency=10):
        ...
        self.filtered_state_pub = rospy.Publisher('/kf/state', Odometry, queue_size=5)
        self.IMU_innovation_publisher = rospy.Publisher('/kf/imu_innovation', Quaternion, queue_size=10)
        self.GNSS_innovation_publisher = rospy.Publisher('/kf/gnss_innovation', Quaternion, queue_size=10)
    ...
    def register_sensors(self, imu_measurements, gnss_measurements):
        ...
        for i in range(len(self.imu_measurements)):
            self.imu_subscribers.append(rospy.Subscriber(self.imu_measurements[i].topic_name, Imu, self.imu_measurements[i].callback))

        for i in range(len(self.gnss_measurements)):
            self.gnss_subscribers.append(rospy.Subscriber(self.gnss_measurements[i].topic_name, NavSatFix, self.gnss_measurements[i].callback))
    ...
    def correct_imu(self, imu_N):
        ...
        rospy.loginfo('CORRECTION OF IMU!!')
